...
class NotificationService:
    ...
    def send_owner_notification(data):
        print(f"NOTIFICATION TO OWNER ({data.get('ownerPhone')}): {data.get('title')} - {data.get('message')}")
    ...
    def get_notifications(phone):
        ...
        if owner:
            notifications = Notification.objects.filter(recipient_phone__iexact=owner.owner_id).order_by('-created_at')
        else:
    ...
    def mark_all_notifications_read(phone):
        ...
        notifications = Notification.objects.filter(recipient_phone__iexact=target_phone, is_read=False)
